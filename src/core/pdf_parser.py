...
class PdfParser():
    ...
    def _obtain_supplier(self):
        ...
        supplier_keys = ["Vendor", "Supplier", "Provider", "From", "Sold By", "Remit To", "Seller"] 
    ...
    def _crop_to_header(self, page):
        ...
        """
        header_keywords = [
                "sku", "qty", "quantity", "description", "desc", 
                "unit price", "price", "unit", "total", "amount", 
                "item", "ref", "part number"
            ]
        header_keywords = [
            "sku", "qty", "quantity", "description", "unit price", 
            "price", "unit", "amount", "item", "part number"
        ]
    ...
    def _clean_table(self):
        if self.po_table.empty: return

        # 1. Standardize headers 
        self.po_table.columns = self.po_table.columns.astype(str).str.replace(r'\n', '').str.strip()
        ...
        # 2. Drop price/total
        cols_to_drop = [c for c in self.po_table.columns if any(x in c.upper() for x in ['PRICE', 'TOTAL', 'AMOUNT', 'COST'])]
        ...
        def is_header_bad(col_name):
            ...
            return name == 'nan' or 'unnamed' in name or name == '' or name == 'none' or name.startswith('.')
        ...
        if len(self.po_table.columns) > 0:
            ...
            self.po_table = self.po_table[self.po_table[first_col].astype(str) != str(first_col)]

        # 6. Final cleanup
        self.po_table = self.po_table.replace(r'\n', '', regex=True)
        # Fix split words (e.g. "G asket")
        self.po_table = self.po_table.replace(r'(?<=[a-zA-Z])\s(?=[a-z])', '', regex=True)
        ...
        self.po_table = self.po_table.apply(lambda x: x.str.strip() if x.dtype == "object" else x)
